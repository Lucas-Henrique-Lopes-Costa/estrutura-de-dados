...
bool tabelaHash::insere(const string &c, char &t, const int &v)
{
    ...
    {
        cout << "\nÉ necessário redimensionar de " << numPosicoes << " para " << numPosicoes + 3 << endl
             << endl;
        redimensiona();
    }
    return true;
...
void tabelaHash::redimensiona()
{
    ...
    for (int i = 0; i < numPosicoes - 3; i++)
    { // Percorre a tabela hash antiga
        while (!tabela[i].vazia())
        {                                                  // Enquanto a lista na posição i não estiver vazia
