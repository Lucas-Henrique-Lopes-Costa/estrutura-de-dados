/*
Questão 7: Fila com armazenamento em vetor - Lista de processos em uma repartição pública - colocar na frente da fila processos de um dado tipo
Implemente uma função que encontre todos os processos de um determinado tipo e coloque-os na frente da fila (sem quebrar a estrutura). A função deve verificar quando a fila está vazia.
Você pode (
e deve!
) utilizar o código que você desenvolveu na atividade "Fila com armazenamento em vetor - Lista de processos em uma repartição pública" como base.
Com essa finalidade, você deverá adicionar mais um comando, como segue:
c seguindo de um carácter: para encontrar todos os processos de um determinado tipo e colocá-los na frente da fila .
Saídas:
Todas as saídas de comandos já estão implementadas na função principal código entregue, somente falta implementar a chamada para a remoção. Ao terminar a execução do programa, todos os itens da pilha são desempilhados e escritos.
Exemplo de Entrada e Saída juntas:
r
Erro: fila vazia!
i processo2 Maria c 5
i processo1 Joao a 2
i processo3 Severino d 9
i processo5 Barnabe a 10
i processo6 Tiao z 22
i processo7 Marlon a 40
i processo4 Marcos f 12
Erro: fila cheia!
r
Nome: processo2 Assunto: Maria Tipo: c Processo: 5
e
Nome: processo1 Assunto: Joao Tipo: a Processo: 2
i processo4 Marcos f 12
c a
f
Nome: processo1 Assunto: Joao Tipo: a Processo: 2
Nome: processo5 Assunto: Barnabe Tipo: a Processo: 10
Nome: processo7 Assunto: Marlon Tipo: a Processo: 40
Nome: processo3 Assunto: Severino Tipo: d Processo: 9
Nome: processo6 Assunto: Tiao Tipo: z Processo: 22
Nome: processo4 Assunto: Marcos Tipo: f Processo: 12
*/

#include <iostream>
using namespace std;

int main() {
    // Seu código aqui
    return 0;
}
